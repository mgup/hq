    ...
    $.getJSON root+'ajax/group_exams', {
      ...
      unless data.disciplines.present()
        ...
        $(data.disciplines).map ->
          dl.append($("<br><dt>#{this.name} <button title='Редактировать дисциплину' class='editDiscipline btn btn-default btn-xs' data-value=#{this.id}><span class='glyphicon glyphicon-edit'></span></button></dt><br>")).text()
          ...
          $(this.exams).map ->
            dd.children('table').append("<tr><td>#{this.id}</td><td>#{this.name}</td>
                                         ...
                                         <td><a href='#' class='btn btn-info' title='Распечатать ведомость'><span class='glyphicon glyphicon-print'></span></a></td>
