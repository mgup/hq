prawn_document margin: [28.34645669291339, 28.34645669291339,
  ...
  count.times do |i|
    ...
    pdf.bounding_box [pdf.bounds.left + pdf.bounds.width*9/19, pdf.bounds.top - 10], width: pdf.bounds.width*10/19 do
      ...
      pdf.font_size 12 do
        pdf.move_down 20
        pdf.text "Выдана <u>#{@student.person.full_name(:dp)}</u>#{birth} о том, что #{@student.sex} #{(@student.is_valid? || @student.student_group_status == Student::STATUS_SABBATICAL) ? 'является' : (@student.person.male? ? 'являлся' : 'являлась')} обучающимся <u>#{@student.group.course}</u> курса#{tax} <u>#{study_form_name(@student.group.form, :rp)}</u> формы обучения по #{@student.group.speciality.name_tvor} <u>#{@student.group.speciality.code}</u> — «<u>#{@student.group.speciality.name}»</u>#{' ' +institute.join(' ')} Московского политехнического университета Высшей школы печати и медиаиндустрии#{license}.", inline_format: true, align: :justify, leading: 6
      end
    end
    pdf.font_size 12 do
        ...
        pdf.move_down 10
        pdf.table data, header: true, width: pdf.bounds.width, cell_style: { padding: [2, 0], border_color: 'ffffff' } do
