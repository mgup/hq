class Study::SubjectsController < ApplicationController
  ...
  def create
    ...
    # subject = Study::Subject.where(year: study_subject[:year]).where(semester: study_subject[:semester]).where(group_id: study_subject[:group_id]).where(title: study_subject[:title]).where(kind: study_subject[:kind]).first
    # if subject
