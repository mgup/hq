class Office::OrdersController < ApplicationController
  ...
  def create_order_with_students(template, students)
    ...
    order = Office::Order.create status: 1, responsible: current_user.positions.where(acl_position_role: Office::Order::RESPONSIBLE_POSITION_ROLES).order(:id).first.id, order_template: template,
