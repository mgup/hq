...
LIMIT 1 ")
  ...
  def target?
    (entrant && entrant.packed_application.competitive_group_target_item_id.present?)
  end
    ...
    doc = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
      xml.file do
        ...
        xml.fileInfo do
          ...
          xml.fileSender '028'
          xml.version '1.1.3'
          xml.recordCount self.all.find_all { |s| s.last_status_order && [1,2,3,16,17,25,10,46,41,11,26,24,28,14,15,20,42,40].include?(s.last_status_order.order_template) && s.last_status_order.order_signing >= Date.new(2016, 8, 24) && s.last_status_order.order_signing <= Date.new(2016, 9, 14) }.length
        end
        xml.recordList do
          xml.parent.namespace = nil
          # убрать find_all
          self.all.find_all { |s| s.last_status_order && [1,2,3,16,17,25,10,46,41,11,26,24,28,14,15,20,42,40].include?(s.last_status_order.order_template) && s.last_status_order.order_signing >= Date.new(2016, 8, 24) && s.last_status_order.order_signing <= Date.new(2016, 9, 14) }.each_with_index do |student, index|
            xml.record do
