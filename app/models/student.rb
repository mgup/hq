    ...
    doc = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
      xml.file do
        ...
        xml.fileInfo do
          ...
          xml.recordCount self.all.find_all { |s| student.group.speciality.master? && s.last_status_order && [16].include?(s.last_status_order.order_template) && s.last_status_order.order_signing >= Date.new(2016, 3, 1) && s.last_status_order.order_signing <= Date.new(2016, 8, 28) }.length
        end
        xml.recordList do
          xml.parent.namespace = nil
          # убрать find_all
          self.all.find_all { |s| student.group.speciality.master? && s.last_status_order && [16].include?(s.last_status_order.order_template) && s.last_status_order.order_signing >= Date.new(2016, 3, 1) && s.last_status_order.order_signing <= Date.new(2016, 8, 28) }.each_with_index do |student, index|
            xml.record do
