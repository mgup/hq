class Entrance::CompetitiveGroupItem < ActiveRecord::Base
  ...
  has_many :competitive_group_item_profiles, class_name: 'Entrance::CompetitiveGroupItemProfile',
           foreign_key: :item_id
  ...
  scope :for_7_july, -> { where('competitive_group_id IN (516482,516483,513387,513377,516434,516435,516490,516481,513408,513376,513372,513371,516491)') } # кроме 516436,516476,516477 (Крым + иностранцы, журналисты)
  scope :for_10_july, -> { joins(:competitive_group)
                           .where('competitive_groups.id NOT IN (516482,516483,513387,513377,516434,516435,516490,516481,513408,513376,513372,513371,516491)
                          AND competitive_groups.campaign_id = 12016') }
  ...
  scope :for_master_10_august, -> { where(competitive_group_id: [516390, 516394,516395,516396,516402,516403,516407,516409,516492]) }
